...
sub new
{
    ...
    my $self = {channel_id => 3, exchange_name => 'cvt.direct', bind_exchange_name => 'cvt.topic', request_id => 'oaindicator', queue_name => 'oa.export',
...
sub init
{
    ...
    $mq->queue_bind ($self->{'channel_id'}, $self->{'queue_name'}, $self->{'bind_exchange_name'}, 'export.' . $self->{'request_id'} . '.#');
    $self->{'mq'} = $mq;
}
